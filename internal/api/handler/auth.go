...
func (h *AuthHandler) Logout(c *fiber.Ctx) error {
	...
	if body.RefreshToken != "" {
		if err := h.redis.Del(context.Background(), fmt.Sprintf("refresh:%s", body.RefreshToken)).Err(); err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "failed to invalidate refresh token"})
		}
	}
	...
	if token := middleware.BearerToken(c); token != "" {
		if err := h.redis.Set(context.Background(), "blacklist:"+token, "1",
			...
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "failed to invalidate access token"})
		}
	}
